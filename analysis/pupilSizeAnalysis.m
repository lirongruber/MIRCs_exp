...
%     end
    ...
    means_stds=[];
        for d=1:7
            p=[];
            for f=1:size(pupil,1)
                p(f,1:size(pupil{f,d},2))=pupil{f,d};
            end
            p(p==0)=nan;
            mean_p(d,1:size(nanmean(p,1),2))=nanmean(p,1);
            subplot(2,4,d)
            plot(1:4:size(mean_p(d,:),2)*4,mean_p(d,:),'Color',currcolor{group})
            hold on
            plot(200, mean(mean_p(d,1:175)),'*','Color',currcolor{group})
            %             text(200,1200+group*100, ['mean=' num2str(round(mean(mean_p(d,1:200)))) '+-' num2str(round(std(mean_p(d,1:200))))],'Color',currcolor{group})
            axis([0 700 1000 1700])
            title(['fixation ' num2str(d)])
            means_stds(d,:)=[mean(mean_p(d,1:175)) std(mean_p(d,1:175))];
        end
        subplot(2,4,8)
        errorbar(1:7,means_stds(:,1),means_stds(:,2),'Color',currcolor{group})
        hold on
        xlabel('fixation')
        axis([0 8 1000 1700])

...
% colors={[74,77,255]./255,[246,75,75]./255,[74,77,255]./255,[246,75,75]./255}; % blue yellow green orange
...
%     errorbar(nanmean(optNum_dav_rev),nanstd(optNum_dav_rev)./sum(~isnan(optNum_dav_rev),1),'color',colors{c})
    ...
    plot(nanmean(optNum_fpca),'color',colors{c},'LineWidth',5)
    xlabel('fixation number from trial start')
    subplot(2,4,3)
    hold on
    ...
    plot(nanmean(optNum_fpca_rev),'color',colors{c},'LineWidth',5)
    xlabel('reversed fixation number from trial end')
    subplot(2,4,7)
    hold on
...
%     
    ...
    histogram (TimeFixation,'FaceColor',colors{c},'BinEdges',0:100:3000,'normalization','probability','FaceAlpha',0.5)
    ...
    histogram (numOfinfoRec,'FaceColor',colors{c},'BinEdges',0:100:2000,'normalization','probability','FaceAlpha',0.5)

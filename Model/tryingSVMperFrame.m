...
%                     load([movies_path '\' folder{1,1} '\' currMov.name], 'filt_movie');
                    load([movies_path '\' folder{1,1} '\' currMov.name], 'rel_movie_notCorr'); filt_movie=rel_movie_notCorr;
                    relFixation=min(FixationNumToUse,size(filt_movie,2));
                    currT=filt_movie{1,relFixation}(:,:,2);
                    if size(currT(currT~=0)',2)>100
        ...
        forSVM={ forSVM{1,:} forSVM{3,:} ; forSVM{2,:}  forSVM{4,:} };
